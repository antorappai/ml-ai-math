    ...
    <div className="page chapter-page">
      ...
      <section className="lesson-map">
        {lessons.map((lesson, index) => {
          const complete = ["basics", "core", "advanced"].every((level) => mastery.completedLevels[lesson.id]?.[level]);
          return <article className="lesson-row" key={lesson.id}><span className="lesson-number">{String(index + 1).padStart(2, "0")}</span><div className="lesson-row-copy"><p className="eyebrow">{lesson.tags.slice(0, 3).join(" · ")}</p><h2>{lesson.title}</h2><p>{lesson.subtitle}</p><div className="lesson-row-action"><Link to={`/lessons/${lesson.id}/study`}>Open lesson</Link><span>{complete ? "Completed" : "Not completed"}</span></div></div>{lesson.projectIds.length > 0 && <span className="project-flag">Project linked</span>}</article>;
        })}
      </section>
